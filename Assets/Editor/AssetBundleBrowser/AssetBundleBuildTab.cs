...
namespace UnityEngine.AssetBundles
{
    ...
    public class AssetBundleBuildTab
    {
        ...
        // gui vars
        [SerializeField]
        private ValidBuildTarget m_BuildTarget = ValidBuildTarget.StandaloneWindows64;
        [SerializeField]
        private CompressOptions m_Compression = CompressOptions.StandardCompression;
        private string m_OutputPath = string.Empty;
        [SerializeField]
        private bool m_UseDefaultPath = true;
        private string m_streamingPath = "Assets/StreamingAssets/TEngine";
        ...
        public void OnGUI(Rect pos)
        {
            ...
            if (GUILayout.Button("Build"))
            {
                ...
                GetAssetBundleMeta();
                CreateConfig();
                EditorTools.AssetBundleToStreamingAssets();
            }
        ...
        void GetAssetBundleMeta()
        {
            JArray abArray = new JArray();
            var bundleList = AssetBundleModel.Model.DataSource.GetAllAssetBundleNames();
            for (int i = 0; i < bundleList.Length; i++)
            {
                JObject abObj = new JObject();
                abObj.Add("abName", bundleList[i].ToString());
                string[] assets = AssetBundleModel.Model.DataSource.GetAssetPathsFromAssetBundle(bundleList[i].ToString());
                JArray assetsArray = new JArray(assets);
                abObj.Add("assets", assetsArray);

                //for (int j = 0; j < assets.Length; j++)
                //{
                //    Debug.Log("          assets:" + assets[j].ToString());
                //}
                string[] dependencies = AssetDatabase.GetAssetBundleDependencies(bundleList[i].ToString(), true);
                JArray dependArray = new JArray(dependencies);
                abObj.Add("depend", dependArray);
                abArray.Add(abObj);
                //if (dependencies.Length > 0)
                //{
                //    for (int k = 0; k < dependencies.Length; k++)
                //    {
                //        Debug.Log("                           depend:" + dependencies[k].ToString());
                //    }
                //}
            }
            System.IO.File.WriteAllText(m_OutputPath + "/AssetBundleMeta.bin", abArray.ToString());
        }

        void CreateConfig()
        {
            JArray array = new JArray();
            DirectoryInfo folder = new DirectoryInfo(m_OutputPath);
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                {
                    long length = new FileInfo(files[i].FullName).Length;
                    string md5 = TEngine.Runtime.Utility.Holin.Get_File_MD5Form_Block(files[i].FullName);
                    JObject obj = new JObject();
                    string fileName = Path.GetFileName(files[i].FullName);
                    obj.Add("fileName", fileName);
                    obj.Add("md5", md5);
                    obj.Add("fileSize", length);
                    array.Add(obj);
                }
                else
                {
            ...
            JObject objTarget = new JObject();
            objTarget.Add("_target", array);
            System.IO.File.WriteAllText(m_OutputPath + "/Md5List.json", objTarget.ToString());

            Debug.Log("get md5 success.");
        }
