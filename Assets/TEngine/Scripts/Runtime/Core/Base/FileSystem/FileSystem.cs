...
namespace TEngine.Runtime
{
    ...
    {
        public const string ArtResourcePath = Constant.Setting.ArtResourcesPath;
        ...
        {
            //默认用外部目录
            string path = FilePath($"{ResourceRoot}/{bundlename}");
            if (!File.Exists(path))
            {
                path = $"{ResourceRootInStreamAsset}/{bundlename}";
            }
