...
namespace TEngine.Runtime
{
    ...
    {
        public const string AssetRootPath = Constant.Setting.AssetRootPath;
        public const string AssetBundleMeta = Constant.Setting.AssetBundleMeta;
        ...
        public void Load()
        {
            ...
            string data = System.IO.File.ReadAllText(FileSystem.GetAssetBundlePathInVersion(AssetBundleMeta));
            JArray abArray = JArray.Parse(data);
            AssetBundleData assetBundleData;
            AssetBundleData depAssetBundleData;

            for (int i = 0; i < abArray.Count; i++)
            {
                JObject abObj = JObject.Parse(abArray[i].ToString());
                string abName = abObj["abName"].ToString();
                string assets = abObj["assets"].ToString();
                string depend = abObj["depend"].ToString();
                JArray assetsArray = JArray.Parse(assets);
                JArray dependArray = JArray.Parse(depend);
                for (int j = 0; j < assetsArray.Count; j++)
                {
                    _assetPath2BundleDatas.Add(assetsArray[j].ToString(), abName);
                }

                if (!_bundleDatas.TryGetValue(abName, out assetBundleData))
                {
                    assetBundleData = new AssetBundleData(abName);
                    _bundleDatas.Add(abName, assetBundleData);
                    assetBundleData.Dependencies = new AssetBundleData[dependArray.Count];
                }
                else
                {
                    if (assetBundleData.Dependencies == null)
                ...
                assetBundleData.InitAssets(assets);
                for (int d = 0; d < dependArray.Count; d++)
                {
                    if (!_bundleDatas.TryGetValue(dependArray[d].ToString(), out depAssetBundleData))
                    {
                        depAssetBundleData = new AssetBundleData(dependArray[d].ToString());
                        _bundleDatas.Add(dependArray[d].ToString(), depAssetBundleData);
                    }
                    assetBundleData.Dependencies[d] = depAssetBundleData;
                }
            }
        }
