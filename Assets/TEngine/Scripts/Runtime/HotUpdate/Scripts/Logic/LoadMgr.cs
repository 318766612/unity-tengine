    ...
    public class LoadMgr : TSingleton<LoadMgr>
    {
        ...
        public void DownLoadCallBack(int result, GameStatus status, List<LoadResource> files = null)
        {
            ...
            else
            {
                _StopLoadingCheck();
                //网络发生变化，重试继续下载
                //if (result == (int)DownLoadResult.NetChanged || result == (int)DownLoadResult.HeadRequestError || result == (int)DownLoadResult.Md5Wrong)
                //{
                //    _RetryConnect();
                //    return;
                //}
                ...
                switch (result)
                {
                    case (int)DownLoadResult.NetChanged:
                    case (int)DownLoadResult.HeadRequestError:
                    case (int)DownLoadResult.Md5Wrong:
                    case (int)DownLoadResult.DownLoadingError:
                    case (int)DownLoadResult.ReceiveNullData:
                    case (int)DownLoadResult.DownError:
                    case (int)DownLoadResult.ReceiveError:
                        _RetryConnect();
                        return;
